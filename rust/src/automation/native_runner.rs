use super::native_capture::{normalize_native_capture, validate_playable_screenshot_difference};
#[cfg(test)]
use crate::automation::command_executable_digest;
#[cfg(test)]
use crate::automation::native_window::NativeInventoryLimits;
use crate::automation::native_window::{
...
use crate::mainloop::options::{DEFAULT_RESOLUTION_HEIGHT, DEFAULT_RESOLUTION_WIDTH};
...
/// Dispatch the native observer and acceptance commands using this caller's
/// executable as the bounded observer helper. Callers must route helper argv
/// back through this entrypoint; no subject-built observer is substituted.
...
impl NativeObservationSession {
    fn pending_checkpoints(&self, state: &NativeWindowChildState) -> io::Result<Vec<usize>> {
        let Some(plan) = self.proof.checkpoint_plan() else {
            return Ok(Vec::new());
        };
        if state.semantic.trace_record_count == 0 {
            return Ok(Vec::new());
        }
        let bytes = read_regular_file_nofollow_bounded(&self.trace_path, self.trace_limit)
            .map_err(io::Error::other)?;
        let count = usize::try_from(state.semantic.trace_record_count).map_err(io::Error::other)?;
        ...
        if records.len() != count
            || native_window_trace_semantic_snapshot(&records).map_err(io::Error::other)?
                != state.semantic
        {
            return Err(io::Error::other(
                "native checkpoint trace cursor differs from publication",
            ));
        }
        let committed = plan.committed(&records).map_err(io::Error::other)?;
        if committed.len() < self.checkpoint_count {
            return Err(io::Error::other("native committed checkpoints regressed"));
        }
        for record in records
            .iter()
            .filter(|record| record.kind == super::trace::RecordKind::Checkpoint)
            .skip(self.checkpoint_count)
        {
            if record.presentation.as_ref().map(|frame| frame.count)
                != Some(state.committed_presentation)
            {
                return Err(io::Error::other(
                    "native checkpoint missed its committed presentation",
                ));
            }
        }
        Ok(committed[self.checkpoint_count..]
            .iter()
    ...
    fn acknowledge_publication(&mut self, state: &NativeWindowChildState) -> io::Result<()> {
        if !self.pending_checkpoints(state)?.is_empty() {
            return Err(io::Error::other(
                "native checkpoint has no visible committed OS capture",
            ));
        }
    ...
    fn acknowledge_unbound_publication(
        &mut self,
        ...
        if self.pending_checkpoints(state)?.is_empty() {
            ...
            // The compositor may not have caught up with the first client
            // presentation. Keep the barrier closed within the existing child
            // acknowledgement deadline rather than discarding its checkpoint.
            Ok(())
        }
    }
    ...
    fn observe(
        ...
        let checkpoints = self.pending_checkpoints(&state)?;
        if self.first_visible_presentation.is_some()
            && checkpoints.is_empty()
            && self.linked_stage(&state)?.is_none()
        {
        ...
        let diagnostic = serde_json::to_vec_pretty(&serde_json::json!({
            "schema": "uqm-native-observation-diagnostic-v1",
            "state": state,
            "observed": observed,
            "client_contained": observed.os_bounds.contains(state.client_bounds),
        }))
        .map_err(io::Error::other)?;
        write_bytes_atomic_noclobber(
            &self.evidence_root.join(format!(
                "os-observation-{}-{}.json",
                state.committed_presentation, observer.sequence
            )),
            &diagnostic,
        )
        .map_err(io::Error::other)?;
        if !observed.visible || observed.minimized {
            ...
            return self.acknowledge_unbound_publication(&state);
        }
        if self.first_visible_presentation.is_none()
        ...
        let stage = self.linked_stage(&state)?;
        ...
        for action_index in checkpoints {
            self.capture_stage(
                observer,
                &observed,
                binding.clone(),
        ...
        let after = self
            .state_reader
            .read(&self.nonce, identity.pid, self.requested_bounds)
            .map_err(io::Error::other)?;
        if after != state {
            return Err(io::Error::other(
                "native publication changed during capture",
            ));
        }
        self.acknowledge_publication(&state)
    }
    ...
    fn linked_stage(
        &self,
        state: &NativeWindowChildState,
    ) -> io::Result<Option<NativeScreenshotStage>> {
        if !self.proof.requires_linked_floors() {
            return Ok(None);
        }
        let Some(first) = self.first_visible_presentation else {
            return Ok(None);
        };
        let count = state
        ...
        if count == self.acceptance_policy.stable_presentation_floor {
            Ok(Some(NativeScreenshotStage::Stable))
        } else if count >= self.acceptance_policy.playable_presentation_floor
            && !self.playable_captured
            && state.semantic.accepted_player_inputs > 0
            && state.semantic.verified_battle_frames >= self.acceptance_policy.battle_frame_floor
        {
            Ok(Some(NativeScreenshotStage::Playable))
        } else {
            Ok(None)
        }
    }
    ...
    fn capture_stage(
        ...
        semantic: crate::automation::NativeWindowSemanticSnapshot,
        stage: NativeScreenshotStage,
    ) -> io::Result<()> {
        let filename = match stage {
            NativeScreenshotStage::Stable => "stable.png".to_string(),
            NativeScreenshotStage::Playable => "playable.png".to_string(),
            NativeScreenshotStage::Checkpoint { action_index } => {
                format!("checkpoint-{action_index:04}.png")
            }
        };
        ...
        let captured = observer
            ...
            .map_err(io::Error::other)?;
        let raw_path = path.with_extension("os.png");
        write_bytes_atomic_noclobber(&raw_path, &captured).map_err(io::Error::other)?;
        let original_os_capture =
            retained_input(&self.evidence_root, &raw_path).map_err(io::Error::other)?;
        ...
        if let NativeScreenshotStage::Checkpoint { action_index } = stage {
            let plan = self
                .proof
                .checkpoint_plan()
                .ok_or_else(|| io::Error::other("missing native checkpoint plan"))?;
            if let Some(previous) = plan
                .previous_capture_for_change(action_index)
                .map_err(io::Error::other)?
            {
                let previous = read_regular_file_nofollow_bounded(
                    &self
                        .screenshots
                        .join(format!("checkpoint-{previous:04}.png")),
                    capture_budget,
                )
                .map_err(io::Error::other)?;
                super::native_capture_change::validate_change(&previous, &bytes, capture_budget)
                    .map_err(io::Error::other)?;
            }
        }
...
struct ResolvedNativeSources {
    shared: Option<super::native_artifacts::SharedInputs>,
    executable: PathBuf,
    script: PathBuf,
    content_package: PathBuf,
    content_filename: std::ffi::OsString,
    retained_script_name: std::ffi::OsString,
}
...
fn resolve_native_sources(
    ...
    script: &Path,
    limit: u64,
) -> Result<ResolvedNativeSources, String> {
    let shared = std::env::var_os("UQM_NATIVE_SHARED_INPUTS")
        .map(|value| {
            let value = value
                .into_string()
                .map_err(|_| "non-UTF8 shared input binding".to_string())?;
            serde_json::from_str::<super::native_artifacts::SharedInputs>(&value)
                .map_err(|error| format!("parse shared input binding: {error}"))
        })
        .transpose()?;
    if let Some(shared) = &shared {
        shared.validate(root)?;
        write_json_atomic(&root.join("shared-inputs.json"), shared)?;
    }
    let retained_script_name = script
        .file_name()
        .ok_or("script has no filename")?
        .to_os_string();
    let (executable, script, content_package, content_filename) = if let Some(shared) = &shared {
        let script_name = script
            .file_name()
            .and_then(|name| name.to_str())
            .ok_or("invalid script name")?;
        let package_name = shared
            .members
            .keys()
            .find(|name| name.starts_with("inputs/content/packages/"))
            .ok_or("shared content package is missing")?;
        (
            shared.path(root, "inputs/uqm", limit)?,
            shared.path(root, &format!("inputs/{script_name}"), limit)?,
            shared.path(root, package_name, limit)?,
        ...
        let package = find_content_package(&content)?;
        let filename = package
            .file_name()
            .ok_or("content package has no filename")?
            .to_os_string();
        (
            fs::canonicalize(executable).map_err(|error| error.to_string())?,
            fs::canonicalize(script).map_err(|error| error.to_string())?,
            package,
            filename,
        )
    };
    Ok(ResolvedNativeSources {
        shared,
        executable,
        script,
        content_package,
        content_filename,
        retained_script_name,
    })
}

fn retain_runtime_inputs(
    root: &Path,
    shared: Option<&super::native_artifacts::SharedInputs>,
    ...
    if let Some(shared) = shared {
        for destination in [executable.1, script.1, content.1] {
            let logical = destination
                .strip_prefix(root)
                .map_err(|error| error.to_string())?
                .to_str()
                .ok_or("non-UTF8 runtime alias")?;
            shared.link_runtime(root, logical, limit)?;
        }
    } else {
        copy_executable_bounded(executable.0, executable.1, limit)?;
        copy_regular_bounded(script.0, script.1, limit)?;
        copy_regular_bounded(content.0, content.1, limit)?;
    }
    Ok(())
}
...
fn run(inputs: RunInputs<'_>) -> Result<(), String> {
    ...
    let _budget = super::native_budget::begin(root, runtime_contract.inventory_limits)?;
    let mut child_spawned = false;
    let mut child_spawn_attempted = false;
    let result = (|| -> Result<(), String> {
        let ResolvedNativeSources {
            shared,
            executable,
            script,
            content_package,
            content_filename,
            retained_script_name,
        } = resolve_native_sources(root, executable, content, script, linked_build_member_limit)?;
        let RunDirectories {
            inputs,
            content_root,
            content_packages,
        ...
        let retained_script = inputs.join(retained_script_name);
        ...
        retain_runtime_inputs(
            root,
            shared.as_ref(),
            (&executable, &retained_executable),
            (&script, &retained_script),
            (&content_package, &retained_content),
            linked_build_member_limit,
        )?;
        write_bytes_atomic_noclobber(
        ...
        let validated_script = validate_script(document, &retained_script)
            .map_err(|error| format!("validate script: {error}"))?;
        let checkpoint_plan = super::native_checkpoint::NativeCheckpointPlan::derive(
            &script_bytes,
            &validated_script,
        )?;
        ...
        let shared_root = shared
            ...
            .map(|shared| {
                shared
                    .path(root, "inputs/uqm", linked_build_member_limit)
                    .and_then(|path| {
                        path.parent()
                            .and_then(Path::parent)
                            .map(Path::to_path_buf)
                            .ok_or("missing shared root".into())
                    })
            })
            .transpose()?;
        super::native_isolation::restrict_candidate(
            &mut command,
            &config_root,
            &automation,
            shared_root.as_deref(),
        )?;
        super::native_budget::reserve(
            &root.join("stdout.log"),
            runtime_contract.child_stdout_budget_bytes,
        )?;
        super::native_budget::reserve(
            &root.join("stderr.log"),
            runtime_contract.child_stderr_budget_bytes,
        )?;
        ...
        let mut observation = NativeObservationSession {
            ...
            checkpoint_count: 0,
            trace_limit: runtime_contract.inventory_limits.member_bytes,
            trace_path: automation.join("trace.jsonl"),
...
fn create_run_directories(root: &Path) -> Result<RunDirectories, String> {
    ...
    let directories = RunDirectories {
        ...
        automation: root.join("runtime-automation"),
    ...
    for directory in [
        ...
        super::native_budget::reserve_directory(directory)?;
    ...
    write_json_atomic(
        &root.join("config-initial.json"),
        &serde_json::json!({
            "schema": "uqm-native-initial-config-v1", "files": [],
        }),
    )?;
    Ok(directories)
}
...
fn publish_setup_failure(
    ...
    if config_root.exists() {
        if let Err(retention) = retain_and_remove_config(root, runtime_contract.inventory_limits) {
            error.push_str(&format!("; {retention}"));
        }
    }
    if let Err(retention) = retain_automation(root, runtime_contract.inventory_limits) {
        error.push_str(&format!("; {retention}"));
    }
    remove_shared_runtime_links(root)?;
    let retained_files =
...
fn finalize_run(inputs: FinalizeRun<'_>) -> Result<(), String> {
    ...
    let shared_cleanup = remove_shared_runtime_links(root);
    let observer_result = observer_result.and(shared_cleanup);
    let config_retention = retain_and_remove_config(root, runtime_contract.inventory_limits);
    let config_root_removed = !config_root.exists();
    let automation_retention = retain_automation(root, runtime_contract.inventory_limits);
    let observer_result = observer_result
        .and(config_retention)
        .and(automation_retention);
    let retained_automation = automation.with_file_name("automation");
    let automation = retained_automation.as_path();
...
fn remove_shared_runtime_links(root: &Path) -> Result<(), String> {
    let Some(shared) =
        super::native_artifacts::SharedInputs::open(root, MAX_NATIVE_MANIFEST_BYTES)?
    else {
        return Ok(());
    };
    for member in shared.inventory() {
        ...
        match fs::symlink_metadata(&path) {
            Ok(_) => {
                if retained_input(root, &path)? != member {
                    return Err("shared runtime alias changed".into());
                }
...
fn retain_automation(
    root: &Path,
    limits: super::native_window::NativeInventoryLimits,
) -> Result<(), String> {
    let source = root.join("runtime-automation");
    if !source.exists() {
        return Ok(());
    }
    let result = (|| {
        let files = super::native_window::native_artifact_inventory(&source, limits)
            .map_err(|error| format!("inventory runtime automation: {error:?}"))?;
        let destination = root.join("automation");
        super::native_budget::reserve_directory(&destination)?;
        fs::create_dir(&destination).map_err(|error| error.to_string())?;
        for member in files {
            let output = destination.join(&member.relative_path);
            if let Some(parent) = output.parent() {
                super::native_budget::reserve_directory(parent)?;
                fs::create_dir_all(parent).map_err(|error| error.to_string())?;
            }
            copy_regular_bounded(
                &source.join(&member.relative_path),
                &output,
                limits.member_bytes,
            )?;
            if retained_input(&destination, &output)? != member {
                return Err("automation changed during retention".into());
            }
        }
        Ok(())
    })();
    let cleanup =
        fs::remove_dir_all(&source).map_err(|error| format!("remove runtime automation: {error}"));
...
fn retain_and_remove_config(
    root: &Path,
    limits: super::native_window::NativeInventoryLimits,
) -> Result<(), String> {
    let config = root.join("config");
    let retained = (|| {
        let files = super::native_window::native_artifact_inventory(&config, limits)
            .map_err(|error| format!("inventory final native config: {error:?}"))?;
        let destination = root.join("config-final");
        super::native_budget::reserve_directory(&destination)?;
        fs::create_dir(&destination)
            .map_err(|error| format!("claim final native config: {error}"))?;
        for member in &files {
            let output = destination.join(&member.relative_path);
            if let Some(parent) = output.parent() {
                super::native_budget::reserve_directory(parent)?;
            ...
            copy_regular_bounded(
                &config.join(&member.relative_path),
                &output,
                limits.member_bytes,
            )?;
            let actual = retained_input(&destination, &output)?;
            if actual != *member {
                return Err("native config changed during retention".into());
            }
        }
        write_json_atomic(
            &root.join("config-final.json"),
            &serde_json::json!({
                "schema": "uqm-native-final-config-v1", "files": files,
            }),
        )
    })();
    let cleanup =
        fs::remove_dir_all(&config).map_err(|error| format!("remove native config: {error}"));
...
#[cfg(target_os = "macos")]
fn retain_crash_report(root: &Path, pid: u32) -> Result<(), String> {
    let home = std::env::var_os("HOME").ok_or("crash report lookup has no HOME")?;
    ...
    loop {
        if let Ok(entries) = fs::read_dir(&reports) {
            for entry in entries.flatten() {
                ...
                let Ok(bytes) = read_regular_file_nofollow_bounded(&path, 1024 * 1024) else {
                    continue;
                };
                ...
                return write_bytes_atomic_noclobber(&root.join("crash-report.ips"), &bytes);
            }
        }
        if std::time::Instant::now() >= deadline {
            ...
            return write_bytes_atomic_noclobber(
...
#[cfg(not(target_os = "macos"))]
fn retain_crash_report(_root: &Path, _pid: u32) -> Result<(), String> {
    Ok(())
}
...
fn publish_failure_manifest(
    ...
    if context.child.signal.is_some() {
        ...
        if let Err(retention) = retain_crash_report(context.root, context.child.process.pid) {
            return publish_failure_without_crash(
                context,
                contract,
                &format!("{error}; crash diagnostic: {retention}"),
            );
        }
    }
    publish_failure_without_crash(context, contract, error)
}

fn publish_failure_without_crash(
    context: &FailureManifestContext<'_>,
    contract: NativeAcceptanceFailureContract,
    error: &str,
) -> Result<PathBuf, String> {
...
fn validate_shared_linked_build(
    root: &Path,
    shared: &super::native_artifacts::SharedInputs,
    executable: &NativeRetainedInput,
    limit: u64,
) -> Result<(), String> {
    let read = |name: &str| shared.read(root, &format!("inputs/linked-build/{name}"), limit);
    let receipt: NativeLinkedBuildReceipt =
        serde_json::from_slice(&read("linked-build-receipt.json")?)
            .map_err(|error| error.to_string())?;
    validate_linked_build_receipt(&receipt, executable)?;
    validate_linked_build_cargo_messages(&receipt, &read("cargo-messages.jsonl")?)?;
    for expected in [
        ...
        &receipt.cargo_manifest,
        &receipt.cargo_lock,
        &receipt.authority,
        &receipt.canonical_toolchain,
    ] {
        let bytes = shared.read(root, &expected.relative_path, limit)?;
        if bytes.len() as u64 != expected.byte_length || sha256_hex(&bytes) != expected.sha256 {
            return Err("shared linked-build member differs from exact build receipt".into());
        }
    }
    validate_native_linked_build_semantics(
        &read("provider-report.json")?,
        &read("native-build-evidence.json")?,
        &read("gates.json")?,
        &read("canonical-toolchain.json")?,
        &read("Cargo.toml")?,
        &read("Cargo.lock")?,
    )
    .map_err(|error| format!("shared linked-build semantics: {error:?}"))
}
...
fn copy_bounded_reader<R: io::Read>(
    ...
    if expected_length > limit {
        return Err("copy exceeds member bound".into());
    }
    super::native_budget::reserve(destination, expected_length)?;
    ...
    let mut input = input;
    let copied = match io::copy(&mut input.by_ref().take(expected_length), &mut output) {
        Ok(copied) => copied,
        Err(error) => {
            drop(output);
            return copy_failure_cleanup(
    ...
    if copied > limit || copied != expected_length || !matches!(input.read(&mut extra), Ok(0)) {
        drop(output);
        return copy_failure_cleanup(
            destination,
            format!(
...
fn write_bytes_atomic_noclobber(path: &Path, bytes: &[u8]) -> Result<(), String> {
    super::native_budget::reserve(path, bytes.len() as u64)?;
    use std::os::fd::{AsRawFd as _, FromRawFd as _};
...
mod tests {
    ...
    #[test]
    fn native_setup_failure_retains_nonempty_config_before_cleanup() {
        let temporary = tempfile::tempdir().unwrap();
        let root = temporary.path();
        fs::create_dir(root.join("config")).unwrap();
        fs::write(root.join("config/uqm.cfg"), b"volume=17\n").unwrap();
        let policy = NativeAcceptancePolicy {
        ...
        .unwrap();
        assert_eq!(
            fs::read(root.join("config-final/uqm.cfg")).unwrap(),
            b"volume=17\n"
        );
    ...
    #[test]
    fn native_configuration_and_runtime_keep_envelope_names_and_refuse_collisions() {
        let temporary = tempfile::tempdir().unwrap();
        let root = temporary.path();
        fs::create_dir(root.join("config")).unwrap();
        fs::create_dir(root.join("runtime-automation")).unwrap();
        fs::write(root.join("config/native-acceptance.json"), b"config").unwrap();
        fs::write(
            root.join("runtime-automation/native-acceptance-failure.json"),
            b"runtime",
        )
        .unwrap();
        retain_and_remove_config(root, runtime_contract().inventory_limits).unwrap();
        retain_automation(root, runtime_contract().inventory_limits).unwrap();
        assert_eq!(
            fs::read(root.join("config-final/native-acceptance.json")).unwrap(),
            b"config"
        );
        assert_eq!(
            fs::read(root.join("automation/native-acceptance-failure.json")).unwrap(),
            b"runtime"
        );
        fs::create_dir(root.join("config")).unwrap();
        fs::write(root.join("config/native-acceptance.json"), b"replacement").unwrap();
        assert!(retain_and_remove_config(root, runtime_contract().inventory_limits).is_err());
        assert_eq!(
            fs::read(root.join("config-final/native-acceptance.json")).unwrap(),
            b"config"
        );
    ...
    #[test]
    fn native_config_budget_exhaustion_keeps_reserved_failure_diagnostic_and_cleans_scratch() {
        let temporary = tempfile::tempdir().unwrap();
        let root = temporary.path();
        let limits = runtime_contract().inventory_limits;
        let _scope = super::super::native_budget::begin_with_limits(root, limits, limits).unwrap();
        let mut remaining = limits.aggregate_bytes - 2 * 1024 * 1024;
        let mut index = 0;
        while remaining > 0 {
            let bytes = remaining.min(limits.member_bytes);
            super::super::native_budget::reserve(&root.join(format!("claimed-{index}")), bytes)
                .unwrap();
            remaining -= bytes;
            index += 1;
        }
        fs::create_dir(root.join("config")).unwrap();
        fs::write(root.join("config/uqm.cfg"), b"nonempty").unwrap();
        let error = retain_and_remove_config(root, limits).unwrap_err();
        assert!(!root.join("config").exists());
        assert!(!root.join("config-final/uqm.cfg").exists());
        publish_setup_failure(
            root,
            1,
            &"a".repeat(64),
        ...
        .unwrap();
        let manifest: NativeAcceptanceSetupFailureManifest =
            serde_json::from_slice(&fs::read(root.join("native-acceptance-failure.json")).unwrap())
                .unwrap();
        assert!(!manifest.passed);
        assert_eq!(manifest.error, error);
        validate_native_acceptance_setup_failure_bundle(root, &manifest).unwrap();
    }
    ...
    fn child_state(
        ...
        NativeWindowChildState {
            schema: crate::automation::NATIVE_WINDOW_STATE_SCHEMA.to_string(),
            nonce: nonce.to_string(),
            pid,
            sdl_window_id: 1,
    ...
    fn final_publication_session(
        ...
        let session = NativeObservationSession {
            ...
            checkpoint_count: 0,
            trace_limit: runtime_contract().inventory_limits.member_bytes,
            trace_path: root.join("automation/trace.jsonl"),
            publications: vec![NativeWindowPublication {
                acknowledgement,
    ...
    #[test]
    fn checkpoint_publication_cannot_be_acknowledged_without_its_os_capture() {
        use crate::automation::trace::{CheckpointEvidence, PresentationEvidence, RecordKind};
        let root = tempfile::tempdir().unwrap();
        let bounds = runtime_contract().expected_client_bounds;
        let nonce = "a".repeat(64);
        let prior = child_state(&nonce, 123, bounds, 0, 0, 0);
        let mut state = child_state(&nonce, 123, bounds, 1, 0, 0);
        let (mut session, identity) = final_publication_session(root.path(), &state, &prior);
        let source = br#"{"version":1,"name":"capture","budgets":{"max_input_ticks":100,"max_presentations":100,"max_wallclock_seconds":60},"steps":[{"action":"capture","label":"menu"},{"action":"finish"}]}"#;
        let script =
            validate_script(parse_script(source, "script.json").unwrap(), "script.json").unwrap();
        let plan =
            crate::automation::native_checkpoint::NativeCheckpointPlan::derive(source, &script)
                .unwrap();
        let frame = PresentationEvidence {
            count: 1,
            generation: 1,
            width: 320,
            height: 240,
        };
        let mut capture = crate::automation::capture_trace_record(
            1,
            0,
            crate::automation::scheduler::CaptureGeneration(1),
            "menu",
        );
        capture.presentation = Some(frame);
        let mut presented = capture.clone();
        presented.sequence = 0;
        presented.kind = RecordKind::Presentation;
        let mut marker = capture.clone();
        marker.sequence = 2;
        marker.kind = RecordKind::Checkpoint;
        marker.checkpoint = Some(CheckpointEvidence {
            id: plan.checkpoints[0].id.clone(),
        });
        let records = vec![presented, capture, marker];
        fs::write(
            root.path().join("automation/trace.jsonl"),
            records
                .iter()
        ...
        .unwrap();
        state.semantic = native_window_trace_semantic_snapshot(&records).unwrap();
        fs::write(
            root.path().join("automation/native-window-state.json"),
            serde_json::to_vec(&state).unwrap(),
        )
        .unwrap();
        session.proof = NativeWindowProof::new(
            native_identity(&identity, &nonce),
            bounds,
            acceptance_policy(),
        )
        .for_script(plan);
        assert_eq!(session.pending_checkpoints(&state).unwrap(), vec![0]);
        assert!(session.acknowledge_publication(&state).is_err());
        assert!(session.acknowledge_unbound_publication(&state).is_ok());
        assert!(session.observe_final_publication(&identity).is_err());
        assert_eq!(session.last_presentation, 0);
        assert!(!root
            .path()
            .join("automation/native-window-ack.json")
            .exists());
        let mut changed = records;
        changed[2].checkpoint.as_mut().unwrap().id.push('x');
        fs::write(
            root.path().join("automation/trace.jsonl"),
            changed
                .iter()
        ...
        .unwrap();
        assert!(session.pending_checkpoints(&state).is_err());
    }

    #[test]
    fn prebinding_acknowledgement_records_and_checks_semantic_counters() {
        let root = tempfile::tempdir().unwrap();
        let nonce = "a".repeat(64);
        let bounds = runtime_contract().expected_client_bounds;
        let prior = child_state(&nonce, 123, bounds, 9, 3, 301);
        let next = child_state(&nonce, 123, bounds, 10, 8, 309);
        let (mut session, identity) = final_publication_session(root.path(), &next, &prior);
        session.acknowledge_unbound_publication(&next).unwrap();
        ...
        let regressed = child_state(&nonce, 123, bounds, 11, 7, 310);
        assert!(session.acknowledge_publication(&regressed).is_err());
