...
mod native_replay;
mod native_suite;
use uqm_rust::automation::native_runner as native_acceptance_runner;
...
/// Controller executable entry, shared by the thin xtask binary.
...
fn run() -> Result<(), String> {
    ...
    if command == "native" {
        ...
        return native_cli::run(&root, &extra);
    }
    if command == "native-suite-validate" {
        let [bundle] = extra.as_slice() else {
            return Err("usage: native-suite-validate BUNDLE (exact selection via UQM_CI_AUTOPLAY_SCENARIOS)".into());
        };
        return validate_native_suite_bundle(&root, Path::new(bundle), None);
    }
    if !extra.is_empty() {
        return Err(format!(
...
/// Validate a retained native suite bundle offline.
///
/// The selection must be the one the run was asked for. It comes from the
/// caller when the runner command supplies it and from the plan's environment
/// binding otherwise, so a bundle cannot be validated against a suite nobody
/// requested.
fn validate_native_suite_bundle(
    root: &Path,
    bundle: &Path,
    ...
    let environment = env::var(ci::exec::AUTOPLAY_SCENARIOS_ENV).ok();
    let selected = selected_acceptance_scripts(&authority, requested.or(environment.as_deref()))?;
    let manifest = native_suite::validate_accounting(bundle, &selected)?;
    if !manifest.passed {
        native_suite::validate_failure_diagnostics(bundle, &authority)?;
    }
    println!(
        "{}",
    ...
    if manifest.passed {
        ...
        Err("native suite diagnostics validated; gameplay suite failed".into())
    }
}

fn usage() -> String {
    "usage: cargo run --manifest-path rust/xtask/Cargo.toml -- <debug|release|test|native|native-test|probe|harness|package|production|prove|verify|capture-dependencies|doctor|matrix|ci>".into()
}
...
fn test_all(root: &Path, run_native_acceptance: bool) -> Result<(), String> {
    ...
    if run_native_acceptance {
        run_native_window_acceptance(
            root,
            &linked_build_proof,
            &NativeRunRequest::from_environment(),
        )?;
    }
    Ok(())
}

/// Where one native acceptance run gets its selection, evidence and content.
///
/// The CI route binds all three through the environment; the `native run`
/// command binds them through options. Both produce the same request so there
/// is one execution path and one evidence layout, not two.
...
impl NativeRunRequest {
    fn from_environment() -> Self {
        Self {
            scenarios: env::var(ci::exec::AUTOPLAY_SCENARIOS_ENV).ok(),
            evidence_root: env::var_os("UQM_CI_NATIVE_ACCEPTANCE_EVIDENCE_ROOT").map(PathBuf::from),
            content_root: env::var_os("UQM_CI_NATIVE_CONTENT_ROOT").map(PathBuf::from),
        }
    }
}

fn native_test(root: &Path) -> Result<(), String> {
    native_test_with(root, &NativeRunRequest::from_environment())
}

pub(crate) fn native_test_with(root: &Path, request: &NativeRunRequest) -> Result<(), String> {
    ...
    let authority = ci::authority::load_authority(root)?;
    ci::authority::validate_authority(&authority)?;
    let selected = selected_acceptance_scripts(&authority, request.scenarios.as_deref())?;
    let mut accounting =
        native_suite::from_request(request.evidence_root.as_deref(), &selected, &authority)?;
    let mut phase = native_suite::SuitePhase::Preflight;
    let result = (|| {
        let admission = native_suite::preflight(root, &selected, &authority)?;
        println!(
            "native suite source admission (not gameplay proof): {}",
            serde_json::to_string(&admission)
                .map_err(|error| format!("serialize native admission: {error}"))?
        );
        phase = native_suite::SuitePhase::Build;
        let cargo_target = ci_cargo_target_dir()?;
        let toolchain = canonical_toolchain(root)
            .map_err(|error| format!("prepare canonical native-test toolchain: {error}"))?;
        prepare_source_environment(root)?;
        prepare_canonical_build(&toolchain)?;
        let linked_build_proof =
            build_linked_test_proof(root, &toolchain, cargo_target.as_deref())?;
        phase = native_suite::SuitePhase::Execute;
        match accounting.as_mut() {
            Some(suite) => {
                run_native_window_acceptance_suite(root, &linked_build_proof, request, suite)
            }
            None => run_native_window_acceptance(root, &linked_build_proof, request),
        }
    })();
    match accounting.as_mut() {
        Some(suite) => native_suite::conclude(suite, result, phase),
        None => result,
    }
}
...
fn run_native_window_acceptance(
    ...
    let Some(mut suite) =
        ...
        println!("native window acceptance skipped: no evidence root was bound; native gameplay was not run");
        return Ok(());
    };
    let result = run_native_window_acceptance_suite(root, linked_build_proof, request, &mut suite);
    native_suite::conclude(&mut suite, result, native_suite::SuitePhase::Execute)
}

fn run_native_window_acceptance_suite(
    root: &Path,
    linked_build_proof: &LinkedBuildProof,
    request: &NativeRunRequest,
    suite: &mut native_suite::SuiteAccounting,
) -> Result<(), String> {
    let Some(evidence_root) = request.evidence_root.clone() else {
        ...
        println!(
            "native window acceptance skipped: no evidence root was bound, through either \
             UQM_CI_NATIVE_ACCEPTANCE_EVIDENCE_ROOT or 'native run --artifacts'. A local run \
             also needs the content root and a logged-in Aqua session, because the acceptance \
             drives a real window."
        );
        return Ok(());
    };
    let content_root = request.content_root.clone().ok_or_else(|| {
        "a content root is required for native acceptance, through either \
         UQM_CI_NATIVE_CONTENT_ROOT or 'native run --content'"
            .to_string()
    })?;
    let authority = ci::authority::load_authority(root)?;
    ci::authority::validate_authority(&authority)?;
    let requested = request.scenarios.clone();
    let selected = selected_acceptance_scripts(&authority, requested.as_deref())?;
    native_suite::preflight(root, &selected, &authority)?;
    ...
    execute_native_scenarios(
        &NativeScenarioLaunch {
            source: root,
            controller: &controller,
            linked_executable: &linked_executable,
...
/// Everything one serial native suite execution launches against.
///
/// The ordinary acceptance route points `source` at the repository and
/// `linked_executable` at the build it just produced. A retained-input replay
/// points both at bytes it staged out of a prior bundle. Nothing else differs,
/// so both go through [`execute_native_scenarios`] rather than through two
/// implementations that can drift apart.
pub(crate) struct NativeScenarioLaunch<'a> {
    /// Root whose `pinned.path` entries hold the exact selected script bytes.
    pub source: &'a Path,
    /// The hashed controller executable, which is also the observer.
    pub controller: &'a Path,
    pub linked_executable: &'a Path,
    pub content_root: &'a Path,
    pub evidence_root: &'a Path,
    pub linked_build_proof: &'a LinkedBuildProof,
    /// The exact scenario list the caller asked for, forwarded to the child.
    pub requested: Option<&'a str>,
}

pub(crate) fn execute_native_scenarios(
    launch: &NativeScenarioLaunch<'_>,
    selected: &[ci::authority::PinnedScript],
    authority: &ci::authority::Authority,
    suite: &mut native_suite::SuiteAccounting,
) -> Result<(), String> {
    let linked_bytes = read_regular_file_nofollow_bounded(
        launch.linked_executable,
    ...
    let shared = prepare_native_shared_inputs(
        launch.evidence_root,
        NativeSharedSources {
            source: launch.source,
            executable: launch.linked_executable,
            content: launch.content_root,
            linked: launch.linked_build_proof,
        },
        selected,
        authority,
        suite.shared_budget(),
    ...
    for (index, pinned) in selected.iter().enumerate() {
        ...
        let (bytes, files) = measure_tree(launch.evidence_root)?;
        ...
        suite.complete(index)?;
    }
    Ok(())
}

fn native_shared_member_names(
    pinned: &ci::authority::PinnedScript,
    authority: &ci::authority::Authority,
) -> Result<Vec<String>, String> {
    let filename = Path::new(&pinned.path)
        .file_name()
        .and_then(|name| name.to_str())
        .ok_or("script filename is not UTF-8")?;
    let mut names = vec![
        "inputs/uqm".into(),
        format!("inputs/{filename}"),
        format!(
            "inputs/content/packages/{}",
            authority.native_acceptance.content_filename
        ),
        "inputs/content/version".into(),
    ];
    names.extend(
        [
            "linked-build-receipt.json",
        ...
        .map(|name| format!("inputs/linked-build/{name}")),
    );
    Ok(names)
}

struct NativeSharedSources<'a> {
    source: &'a Path,
    executable: &'a Path,
    content: &'a Path,
    linked: &'a LinkedBuildProof,
}

fn prepare_native_shared_inputs(
    evidence: &Path,
    sources: NativeSharedSources<'_>,
    selected: &[ci::authority::PinnedScript],
    authority: &ci::authority::Authority,
    budget: uqm_rust::automation::native_artifacts::ArtifactBudget,
) -> Result<uqm_rust::automation::native_artifacts::SharedSnapshot, String> {
    use uqm_rust::automation::native_artifacts::SharedSnapshot;
    let NativeSharedSources {
        source,
        executable,
        content,
        linked,
    ...
    let mut snapshot = SharedSnapshot::create(&evidence.join("shared"), budget)?;
    snapshot.insert(
        "inputs/uqm",
        &read_regular_file_nofollow_bounded(executable, limit)?,
    )?;
    let package = &authority.native_acceptance;
    let bytes =
        read_regular_file_nofollow_bounded(&content.join(&package.content_filename), limit)?;
    if bytes.len() as u64 != package.content_byte_length
        || hex_sha256(&bytes) != package.content_sha256
    {
        return Err("shared content differs from authority".into());
    }
    snapshot.insert(
        &format!("inputs/content/packages/{}", package.content_filename),
        &bytes,
    )?;
    snapshot.insert(
        "inputs/content/version",
        format!("{}\n", package.content_version).as_bytes(),
    )?;
    for pinned in selected {
        native_suite::admit_script(source, pinned, authority)?;
        let name = Path::new(&pinned.path)
            .file_name()
            .and_then(|name| name.to_str())
            .ok_or("invalid script filename")?;
        let bytes =
            read_regular_file_nofollow_bounded(&source.join(&pinned.path), pinned.byte_length)?;
        if bytes.len() as u64 != pinned.byte_length || hex_sha256(&bytes) != pinned.sha256 {
            return Err("script changed before shared snapshot".into());
        }
        snapshot.insert(&format!("inputs/{name}"), &bytes)?;
    }
    for entry in fs::read_dir(linked.directory.path()).map_err(|error| error.to_string())? {
        let entry = entry.map_err(|error| error.to_string())?;
        let name = entry
            .file_name()
            .into_string()
            .map_err(|_| "invalid linked member filename")?;
        snapshot.insert(
            &format!("inputs/linked-build/{name}"),
            &read_regular_file_nofollow_bounded(&entry.path(), limit)?,
        )?;
    }
    snapshot.seal()?;
    Ok(snapshot)
...
fn populate_package(
    ...
    ci::bounded_io::copy_group_readable_executable_nofollow(
        &source,
        &destination,
        executable_limit,
    )
    ...
    ci::bounded_io::write_group_readable_regular_nofollow(
        &manifest_staged,
        &staged_bytes,
        evidence_limit,
    )
...
fn validate_proof_stage(command: &str, has_determinism_proof: bool) -> Result<(), String> {
    if command == PRODUCTION_COMMAND {
        return Err(format!(
            "verify requires the two-clean-build determinism proof; production is a single build. Run `{PROVE_COMMAND}`, then verify; do not edit recorded command provenance"
        ));
    }
    if command != PROVE_COMMAND {
        return Err(
            "artifact proof producing command differs from the required prove command".into(),
        );
    }
    if !has_determinism_proof {
        return Err(format!(
            "artifact manifest lacks mandatory determinism proof; run `{PROVE_COMMAND}`"
        ));
    }
    Ok(())
}
...
fn verify_artifact_manifest_with_authority(
    ...
    validate_proof_stage(&manifest.command, manifest.determinism_proof.is_some())?;
...
mod tests {
    use super::*;
    #[test]
    fn verify_requires_prove_stage_not_a_production_build() {
        let error = validate_proof_stage(PRODUCTION_COMMAND, false).unwrap_err();
        assert!(error.contains("production is a single build"), "{error}");
        assert!(error.contains(PROVE_COMMAND), "{error}");
        assert!(validate_proof_stage(PRODUCTION_COMMAND, true).is_err());
        assert!(validate_proof_stage(PROVE_COMMAND, false).is_err());
        assert!(validate_proof_stage("forged command", true).is_err());
        assert!(validate_proof_stage(PROVE_COMMAND, true).is_ok());
    }
