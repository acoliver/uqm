# Phase 11: Integration Testing + Cleanup
...
- Verify: Event-pump lifecycle verification passed
...
### REQ-RL-001 / REQ-RL-011 event follow-through
This phase keeps event-lifecycle verification in the migrated integration suite so final verification does not rely on P10 alone.
...
17. **Event lifecycle follow-through**: verify the migrated integration harness still has a functioning event path after init and reinit

### Task 2: Remove backup files (G17)
...
17. `test_event_path_survives_reinit_integration` — verifies event-processing availability through the migrated integrated lifecycle

### Verify existing tests still pass
18. Run full test suite: `cargo test --workspace --all-features`
...
- [ ] Integration test file exists with 17+ tests covering migrated-path semantics
- [ ] Backup files removed
- [ ] Canvas API contract rechecked against the actual header/caller surface
- [ ] Deterministic scanline semantic test exists
- [ ] Event-lifecycle follow-through integration test exists
...
- [ ] Event path remains functional through the migrated integrated lifecycle
...
- [ ] Scanline behavior is semantically verified before P12
- [ ] Event-lifecycle behavior remains covered after dedicated P10 verification
