...
use super::c_extern;
use super::do_restart::DoRestartState;
...
/// @plan PLAN-20260707-RESTARTMENU.P07
/// @requirement REQ-RM-007
pub fn try_start_game_impl<O: RestartMenuOps + ?Sized>(ops: &O) -> bool {
    ...
    ops.set_menu_priv_data(menu_ptr, (&mut menu_state as *mut DoRestartState) as usize);
...
/// @plan PLAN-20260707-RESTARTMENU.P07
/// @requirement REQ-RM-007
pub fn start_game_impl<O: RestartMenuOps + ?Sized>(ops: &O) -> bool {
    loop {
...
mod tests {
    ...
    impl RestartMenuOps for MockOps {
        ...
        fn get_menu_input(&self) -> super::super::types::MenuInputState {
            Default::default()
        }
        ...
        fn clear_drawable(&self) {
            self.clear_drawable_called.set(true);
        }
        fn flush_color_xforms(&self) {}
        fn screen_transition(&self, _a: c_int) {}
        fn set_bg_color(&self, _color: Color) {
            self.set_bg_color_called.set(true);
        }
        fn seed_random(&self) {
            self.seed_random_called.set(true);
        }
        ...
        fn melee(&self) {
            self.melee_called.set(true);
        }
        fn setup_menu(&self) {}
        fn free_game_data(&self) {
            self.free_game_data_called.set(true);
        }
        fn introduction(&self) {
            self.introduction_called.set(true);
        }
        fn credits(&self, victory: bool) {
            self.credits_called.set(true);
            self.credits_victory.set(victory);
        }
        fn victory(&self) {
            self.victory_called.set(true);
        }
        ...
        fn set_player_control(&self, player: u8, control: u16) {
            if player == 0 {
                self.player_control_0.set(control);
            } else {
                self.player_control_1.set(control);
            }
        }
        fn assign_global_arrays(&self) {
            self.assign_globals_called.set(true);
        }
    ...
    fn restart_menu_new_game_returns_true() {
        let ops = MockOps::new();
        ops.do_input_activity_after
            .set(in_interplanetary_activity());
    ...
    fn restart_menu_utwig_bomb_path_sets_bg_color() {
        ...
        assert!(
            ops.set_bg_color_called.get(),
            "bg color should be set for utwig bomb path"
        );
        assert!(
            ops.clear_drawable_called.get(),
            "drawable should be cleared"
        );
        assert_eq!(
            ops.utwig_bomb_on_ship.get(),
            0,
            "bomb on ship should be cleared"
        );
    }
    ...
    fn restart_menu_victory_path_calls_victory_and_credits() {
        ...
        assert!(
            ops.credits_victory.get(),
            "Credits should be called with victory=true"
        );
        assert!(
            ops.free_game_data_called.get(),
            "FreeGameData should be called"
        );
    }
    ...
    fn start_game_new_game_calls_introduction() {
        ...
        assert!(
            ops.introduction_called.get(),
            "Introduction should be called for new game"
        );
    }
    ...
    fn start_game_sets_player_controls() {
        ...
        assert!(result, "should return true");
        assert_eq!(
            ops.player_control_0.get(),
            (HUMAN_CONTROL as u16) | (STANDARD_RATING as u16)
        );
        assert_eq!(
            ops.player_control_1.get(),
            (COMPUTER_CONTROL as u16) | (AWESOME_RATING as u16)
        );
    ...
    fn start_game_timeout_shows_splash_and_credits() {
        ...
        assert!(
            ops.splash_called.get(),
            "splash should be called on timeout"
        );
    ...
    const fn in_interplanetary_activity() -> u16 {
        4
    }
}
